...
poly(ger)
scatter!(pts, color=prev, colorrange=extrema(prev), markersize=3log.(germany_df.number_tested .+ 1))
current_figure()
...
L = [Float32.(SDMLayer(RasterData(CHELSA2, BioClim); layer=i, SDT.boundingbox(ger)...)) for i in 1:19]
mask!(L, ger)
...
heatmap(unc)
scatter!(pts[host_idx], color=prev[host_idx] .== 0, colormap=[:grey85, :black], markersize = 15)
current_figure()
...
dist_mat = [SDT.Fauxcurrences._distancefunction(these_pts[i], these_pts[j]) for i in eachindex(these_pts), j in eachindex(these_pts)]
median(dist_mat)
