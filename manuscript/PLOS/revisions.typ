...
#add[The method used to identify infection can also be used as a basis for species weighting: for example, PCR and serology provide qualitatively different information about about the pathogen/host relationship. PCR can only identify an ongoing infection during viral shedding, whereas serology _can_ determine if a pathogen was ever present in a host. However, positive serological tests only indicate that a host was _exposed_ to the pathogen, and does not definitevely indicate the host can or did transmit the pathogen. The form of collected data should be considered, along with these factors, when assigning weights.] #cut[Hosts for a particular pathogen identified through methods with higher false-negative rates (e.g. serology vs. PCR) can be given lower weights.] When there is uncertainty about the capacity of a host to be infected by a pathogen, methods that assign weights based on phylogenetic proximity to known hosts @Elmasri2020HierarchicalBayesian #add[can also provide usable information, particularly as distance to humans is a strong predictor of zoonotic potential @Guth2019HostPhylogenetic]. 

#add[At some level, weights will reflect expert understanding of host-pathogen relationships and judgement of risk. While explicit recommendations of formulae to construct weights from phyologenies, predicted host-virus association networks, and serological evidence remain outside the scope of this paper, future work could apply comparative methods to test the efficacy of different weight-construction methods on simulated data under different levels of data availability to assess the robustness of weight construction in practice.] #cut[Ideally, all aspects of the competence and relevance of hosts for pathogen transmission can be aggregated by experts to establish species weighting —] 

#add[Assessing the robustness of sampling priority to both different weight selection and host inclusion is an essential to confidence this protocol provides the most informative data as possible.] We discuss the use of sensitivity analyses to better understand the consequences of these choices on the recommended configuration of the monitoring network in the final subsection of the protocol. #add[If there is considerable epistemic uncertainty regarding assigned host weights, or inclusion of specific hosts, priority maps from multiple candidate sets of weights can be created, and the regions where sampling priority is high under all candidate priority maps can be used as the region to target sampling].
...
#add[There a numerous factors that impact how prevalence data can be used to inform spatial sampling prioritization. Pathogen prevalence data is often reported at coarse spatial scales associated with regional administrative boundaries.] Alternatively, if existing prevalence data #cut[has] #add[is precise with] high enough spatial coverage that it enables spatially explicit estimates of prevalence, sampling can most effectively improve spatial predictions of prevalence by targeting regions where the degree of prevalence is uncertain, but for which we are confident there is a high biodiversity dose (as we do in the second case study). This is useful for #cut[precisely] locating existing reservoirs across space to better map zoonotic hazard and spillover risk. #add[Another consideration is the sample size from which existing prevalence estimates are derived. For example, prevalence estimates derived from 10 total tested individuals are far more uncertain that 500 total individuals, and this uncertainty can be used to inform sampling priority.] For a full overview of potential methods to use depending on the scale of existing prevalence data see Supplemental Table 1, and for guidelines on how to target different forms of sampling regions based on their predicted dose, dose uncertainty, and predicted prevalence, see Supplemental Table 2.
...
To illustrate this approach, we consider two case studies on _Arena_- and _Hantaviridae_ and their #change[mammalian] hosts, using the global database compiled by #cite(<Simons2025ProtocolProduce>, form: "prose"). The first case study is on #change[rodent and shrew] hosts of _Arenaviridae_ in India, and reflects the scenario where there is no prevalence data available. The second is for #add[_Hantaviridae_] in South Korea, where we have sufficient prevalence data to make spatially explicit predictions of prevalence and use this to guide sampling. 
...
Prevalence #add[information is incorporated into the prioritization by spatially explicitly estimating prevalence] across the extent #cut[is estimated] using Gaussian Process regression (also known as kriging) for each host-virus pair with at least 5 populations sampled across space. This is done using a Squared-Exponential covariance kernel with a fixed length scale parameter using GaussianProcesses.jl @Fairbrother2022GaussianprocessesjlNonparametric, and the hyperparameters for the model are optimized using the L-BFGS algorithm in Optim.jl @Mogensen2018OptimMathematical. This provides both spatial estimates of prevalence, but crucially, spatially explicit uncertainty #add[maps] associated with this prevalence estimate.

@korea shows both the bivariate plot of biodiversity dose vs. dose uncertainty (@korea\A) and dose vs. prevalence uncertainty (@korea\B). #add[
  This identifies a clear geographic trend in dose and uncertainty. Dose is highly concentrated in the southeast of the Korean peninsula, with dose uncertainty (uncertainty regarding the presence of hosts) concentrated in the northwest, near Seoul and many other large cities. In contrast, prevalence uncertainty is primarily concentrated in the region of highest dose, the southeast. This indicates that southeastern Korea is a region for a priority for prevalence sampling, as it contains many hosts with high confidence, but lacks existing data on the prevalence of these pathogens.   
]

To #add[better] isolate regions where we are confident hosts are present for prevalence sampling, and regions of high host presence uncertainty for discovery sampling, we stratify the spatial extent into discrete regions for prevalence sampling and occurrence sampling as follows: the region that is both in the top 50% of biodiversity dose and top 50% of prevalence uncertainty is the stratum for prevalence sampling, and the region in the top 50% of dose uncertainty is the stratum for discovery sampling (@korea\C). Spatial regions that fall into both categories are then targets for both forms of sampling. The choice of 50% quantiles is arbitrary for this example. The sampling priority within each of these strata can be seen in @korea\D. #add[This allows explicit quantification of the regions of higheset priority for each type of sampling, and highlights that more occurrence data for hosts is the highest priority in the northwest, and that prevalence sampling is likely to be most fruitful in the southeast.]

The overall #add[sampling] priority map can be seen in @korea\E,  along with the points sampled using Balanced Acceptance Sampling with a tilting value of $alpha = 2.0$ #add[(see previous case study for discussion of tilting value selection)], with the marker indicating the type of sampling #add[(prevalence, occurrence, or both)] that should take place at each location. 
...
There are numerous future directions for incorporating more sophisticated forms of SDMs to help alleviate these challenges: temporally-explicit SDMs are possible but require much more data at a high temporal resolution, which are typically not available for most species @Anselmetto2025LeveragingLongterm. Similarly, mechanistic SDMs, which directly integrate biological processes into habitat suitability predictions, require far more data than is typically available. Another limitation of conventional correlative SDMs is that habitat suitability does not directly translate to species occupancy. Species may not be present in suitable habitat due to competition or dispersal limitation, and this can impact viral transmission if a host species is replaced by a less competent host (e.g. Lassa fever is reported less in cities where _Mus musculus_ has displaced primary reservoir species). Joint SDMs (JSDMs; #cite(<Pollock2014UnderstandingCooccurrence>, form: "prose")), which model species composition across an entire species pool (thereby accounting for species interactions), are another possible frontier, but JSDMs require explicit co-occurrence data, typically not attainable from platforms like GBIF. Generally, these more informative SDMs are not possible without extensive, long-term #add[species] monitoring, which is why the goal of this protocol is to provide #cut[get these long-term monitoring programs off the ground with] #add[an] efficient #add[standardized protocol for] sampling in the low data context#add[, and to integrate biosurveillance with existing biodiversity monitoring programs].

#add[Operationalizing this protocol will require clearing numerous logistical hurdles; the primary constraint on any biosurveillance or biodiversity monitoring program being funding. Efforts to develop a Global Biodiversity Observing System (GBiOS; #cite(<Gonzalez2023GlobalBiodiversity>, form: "prose")), analogous to the Global Climate Observing System (GCOS), propose long-term funding through a UN coalition fund, similar to the Systematic Observations Financing Facility (SOFF) that funds GCOS. Although many national bodies have developed biosurveillance programs @Sharan2023SurveillanceResponse, ensuring they are working in tandem is essential to maximizing their utility, and this funding model may prove more effective in addressing the trans-national nature of One Health goals, with the aim of creating a confederated system for disease surveillance that builds on existing long-term biodiversity monitoring and biosurveillance programs. Given the considerable overlap between the goals of the Kunming-Montreal Global Biodiversity Framework (GBF) and One Health objectives @Banville2026GlobalBiodiversity, long term funding of GBiOS should also support the integration of biosurveillance programs. Such an integration relies on standards to ensure data is available in interoperable formats @Poisot2025BiodiversityScience to ensure monitoring has the maximum possible benefits @Shanbehzadeh2022DesigningStandardized.  
] #cut[However] An increasing amount of pathogen prevalence data is only actionable when individual test outcomes, both positive and negative, are shared in a way that is both standardized and interoperable with the commonly used representations of biodiversity data @Schwantes2025MinimumData. 

#add[
  Earth's shifting biogeography is driving changes in the distribution of zoonotic pathogens, creating new pathways for spillover that poses a major challenge for public health @Plowright2017PathwaysZoonotic. 
] Further integration of #cut[open] biosurveillance and biodiversity monitoring systems is essential to meet our common goals @Poisot2025BiodiversityScience #add[and mitigate the consequences of this change on human health]. #add[The One Health approach emphasizes the necessity of incorporating the interlinkages between biodiversity and human health, and the protocol we have presented provides a basis to integrate the increasingly available data on Earth's biodiversity into biosurveillance design that directly accounts for the intertwined nature of zoonotic pathogens and biodiversity.] 
Better biosurveillance itself is not #add[sufficient to prevent] spillover #cut[prevention], but it’s a necessary #change[pillar] #cut[of it] to make prevention possible --- it’s the best we can do in the context of limited information on #add[pathogen] prevalence --- but a full approach to prevention of zoonotic spillover is broader than this alone @Ramakrishnan2023BiosurveillanceOne. This protocol represents a necessary #change[next] step toward #add[integrating] #cut[widespread] biosurveillance #cut[integrated ] with global#add[, long-term] biodiversity monitoring systems @Gonzalez2023GlobalBiodiversity to ensure a just planetary future @Chapman2024BiodiversityMonitoring. 
