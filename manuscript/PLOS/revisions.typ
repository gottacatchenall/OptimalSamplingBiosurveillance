...
#change[We address this data limitation by using weighted estimates of host composition to prioritize sampling locations. Further, this determines which type of sampling (species occurrence vs. pathogens prevalence) is most informative at a given location.] While #cut[estimates based only on the] predicted presence of host species #change[does] not fully capture the risk of disease transmission, #change[it hints] at the potential for pathogen presence#change[. This is] especially #change[the case] different species can be weighted by their relative importance as reservoir hosts, i.e. accounting for intraspecific variation in host competence, population density, and probable role in pathogen maintenance.
...
#add[Assessing the robustness of sampling priority to different weight selection is an essential to confidence this protocol provides the most informative data as possible.] We discuss the use of sensitivity analyses to better understand the consequences of these choices on the recommended configuration of the monitoring network in the final subsection of the protocol. #add[If there is considerable epistemic uncertainty regarding assigned host weights, priority maps from multiple candidate sets of weights can be created, and the regions where sampling priority is high under all candidate weightings can be used as the region to target sampling].
