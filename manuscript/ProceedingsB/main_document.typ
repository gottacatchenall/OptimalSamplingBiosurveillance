...
Alternatively, if existing prevalence data has high enough spatial coverage that it enables spatially explicit estimates of prevalence, sampling can most effectively improve spatial predictions of prevalence by targeting regions where the degree of prevalence is uncertain, but for which we are confident there is a high biodiversity dose (as we do in the second case study). This is useful for precisely locating existing reservoirs across space to better map zoonotic hazard and spillover risk. For a full overview of potential methods to use depending on the scale of existing prevalence data see Supplemental Table 1, and for guidelines on how to target different forms of sampling regions based on their predicted dose, dose uncertainty, and predicted prevalence, see Supplemental Table 2.
...
In practice, algorithms for generating spatially balanced samples targeted toward regions of high priority may not be skewed “enough” toward high priority sites for a given use case, so we also tend to _tilt_ the distribution of priority scores to make the values of high priority even higher. This is done using a function $T(P_x , alpha )$, defined as 
