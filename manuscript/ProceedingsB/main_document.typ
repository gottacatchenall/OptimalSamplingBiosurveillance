...
Land use and climate change are increasing the risk of zoonotic spillover into human populations. However, we lack actionable information about the pathogen prevalence in wildlife for most of the globe, challenging our ability to prevent spillover. Even when this data exists, it has typically been sampled opportunistically and without guidance based on the relationship between biodiversity and disease. Biosurveillance is essential to mitigating zoonotic spillover, but given the cost of effective monitoring programs, sampling design must be done strategically to obtain as much useful information as possible under fixed budgets. We present a protocol that integrates biodiversity data into zoonoses sampling prioritization based on host species distribution models, with optional potential to integrate pathogen prevalence data (if available). This protocol integrates established practices from biodiversity monitoring to select sampling sites that most efficiently inform our understanding of the state of a pathogen, with the flexibility to adapt to different levels of data availability, based on a principled understanding of host distribution and pathogen biology. We illustrate this with case studies representing both data poor and data rich contexts. We view this as a basis for integrating biosurveillance and biodiversity monitoring programs, and most effectively informing public health decision making.
...
We obtain occurrence records for each rodent host species from the Global Biodiversity Information Facility (GBIF). As predictor variables, we use the 19 bioclimatic variables from CHELSA @Karger2017ClimatologiesHigh at 1km x 1km resolution for South Korea, and from WorldClim @Fick2017Worldclim2 at 2.5 arcminute resolution (approximately 5km x 5km at the equator) for India (to keep the raster size reasonable). We generate pseudoabsences @Barbet-Massin2012SelectingPseudoabsences using background thickening @Vollering2019BunchingBackground, with a buffer around each occurrence record where no pseudoabsences can go (25km for India, 8km for South Korea). 
