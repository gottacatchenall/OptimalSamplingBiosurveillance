...
map_sampling <- function(data = combined, country_iso = "KOR") {
  ...
  host_barchart_prevalence <- ggplot(data = prevalence_raw_data, 
    ...
    geom_point(aes(x = prop_positive),
               ...
               alpha = 0.6,
               size = 2.5) +
    geom_point(data = prevalence_summary_data,
  ...
  host_dashboard <- host_map + host_barchart_tested + host_barchart_prevalence +
    plot_layout(widths = c(2, 1, 1),
                guides = "collect") +
    plot_annotation(title = paste0("Host Surveillance Summary for: ", country_iso)) & 
    theme(legend.position = "bottom")
  
  return(list(pathogen_dashboard,
              host_dashboard))
}
