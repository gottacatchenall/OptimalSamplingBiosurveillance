...
arha_db <- readRDS(here("data", "Project_ArHa_database_2025-09-25.rds"))
...
combined_summary <- combined |>
  ...
  summarise(
    number_detected = sum(number_of_hosts, na.rm = TRUE),
...
map_sampling <- function(data = combined, country_iso = "KOR") {
  country_name <- as.character(unique(combined$country_raw[which(
    combined$iso3c == country_iso
  )]))
  ...
  pathogen_map <- ggplot() +
    ...
    scale_size_continuous(name = "Proportion Positive", range = c(1, 5)) +
    scale_color_viridis_d(name = "Pathogen Species") +
    labs(
      title = paste0("Sampling by pathogen in: ", country_name),
      caption = paste0("Overlain points have been jittered by 10,000 meters around the reported location.")
    ) +
    theme_minimal() +
    theme(legend.position = "bottom")
  ...
  pathogen_dashboard <- pathogen_map + pathogen_barchart_tested + pathogen_barchart_prevalence +
    ...
    plot_annotation(title = paste0("Pathogen Surveillance Summary for: ", country_name)) &
    theme(legend.position = "bottom")
  ...
  host_map <- ggplot() +
    ...
    scale_size_continuous(name = "Proportion Positive", range = c(1, 4)) +
    scale_color_viridis_d(name = "Host Species") +
    labs(title = paste0("Sampling by host in: ", country_name)) +
    theme_minimal() +
    theme(legend.position = "bottom")

  host_barchart_tested <- data_jittered |>
    as_tibble() |>
    group_by(pathogen_species_cleaned, host_species) |>
    summarise(n_tested = sum(n_tested, na.rm = TRUE), .groups = "drop") |>
    filter(n_tested != 0) |>
    ggplot(aes(x = n_tested, y = host_species, fill = pathogen_species_cleaned)) +
    geom_col(colour = "black") +
    scale_fill_viridis_d(name = "Pathogen Species") +
    labs(x = "Total Tested", y = NULL, title = "N Tested") +
    theme_minimal() +
    theme(legend.position = "none")

  prevalence_raw_data <- data_jittered |>
    as_tibble() |>
    filter(n_tested != 0)

  prevalence_summary_data <- prevalence_raw_data |>
    group_by(pathogen_species_cleaned, host_species) |>
    summarise(
      n_tested = sum(n_tested, na.rm = TRUE),
      n_positive = sum(n_positive, na.rm = TRUE),
  ...
  host_barchart_prevalence <- ggplot(
    data = prevalence_raw_data,
    aes(y = host_species, colour = pathogen_species_cleaned)
  ) +
    geom_point(aes(x = prop_positive),
      position = position_jitter(width = 0, height = 0.3),
      alpha = 0.6,
      size = 2.5
    ) +
    geom_point(
      data = prevalence_summary_data,
      aes(x = prop_positive, fill = pathogen_species_cleaned),
      shape = 21,
      size = 4,
      stroke = 1.2
    ) +
    scale_x_continuous(labels = scales::percent) +
    scale_colour_viridis_d(name = "Pathogen Species") +
    scale_fill_viridis_d(name = "Pathogen Species") +
    guides(color = guide_legend(override.aes = list(size = 4))) +
    labs(
      x = "Pathogen Prevalence",
      y = NULL,
      title = "Prevalence",
      caption = "Large circles represent the overall prevalence per host-pathogen pair."
    ) +
    theme_minimal()

  host_dashboard <- host_map + host_barchart_tested + host_barchart_prevalence +
    plot_layout(
      widths = c(2, 1, 1),
      guides = "collect"
    ) +
    plot_annotation(title = paste0("Host Surveillance Summary for: ", country_name)) &
    theme(legend.position = "bottom")
...
map_sampling(data = combined, country_iso = "BEL")
map_sampling(data = combined, country_iso = "KOR")
map_sampling(data = combined, country_iso = "ALB")
map_sampling(data = combined, country_iso = "ARG")
